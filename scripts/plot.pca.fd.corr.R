...
function (x, nx = 128,height=240, pointplot = TRUE, harm = 0, expand = 0, pcweight = NULL,
    cycle = FALSE, land = NA , xlab = NULL, ylab = NULL, landlab = NULL, plots_dir = NULL, basename = NULL,ylog = NULL,...)
{
	...
	if (is.null(pcweight)) 
		...
		png(paste(plots_dir,basename,iharm,'.png',sep=''),height=height)
		...
		ylim = sapply(pcweight[jvar] * range( as.vector( pcmat )),log_fun[[ylog[jvar]]])
		 ...
		 plottype <- "l"
                  percentvar <- round(100 * pcafd$varprop[iharm],
                    1)
		if (!is.null(plots_dir))
		png(paste(plots_dir,basename,iharm,jvar,'.png',sep=''),height=height)
